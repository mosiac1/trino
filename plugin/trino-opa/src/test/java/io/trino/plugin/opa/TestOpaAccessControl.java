...
public class TestOpaAccessControl
{
    ...
    {
        Consumer<OpaAccessControl> methodUnderTest = authorizer -> authorizer.getColumnMask(TEST_SECURITY_CONTEXT, TEST_COLUMN_MASKING_TABLE_NAME, "some_column", VarcharType.VARCHAR);
        assertAccessControlMethodThrowsForIllegalResponses(methodUnderTest, columnMaskingOpaConfig(), OPA_COLUMN_MASKING_URI);

        // Also test a valid JSON response, but containing invalid fields for a colum mask request
        String validJsonButIllegalSchemaResponseContents = """
                {
    ...
    private void testGetColumnMask(String responseContent, Optional<OpaViewExpression> expectedExpression)
    {
        ...
        ColumnSchema columnSchema = createColumnSchema("some_column");

        Optional<ViewExpression> result = authorizer.getColumnMask(TEST_SECURITY_CONTEXT, TEST_COLUMN_MASKING_TABLE_NAME, columnSchema.getName(), VarcharType.VARCHAR);

        assertColumnMaskBehaviour(
                ImmutableList.of("some_column"),
                result.map(viewExpression -> ImmutableMap.of(columnSchema, viewExpression)).orElse(ImmutableMap.of()),
                expectedExpression.map(opaViewExpression -> ImmutableMap.of(columnSchema, opaViewExpression)).orElse(ImmutableMap.of()),
    ...
    @Test
    public void testGetParallelColumnMasks()
    {
        testGetParallelColumnMasks(ImmutableMap.of(createColumnSchema("some-column"), "{}"), ImmutableMap.of());

        String nullResponse = """
                {
                    "result": null
                }""";
        testGetParallelColumnMasks(ImmutableMap.of(createColumnSchema("some-column"), nullResponse), ImmutableMap.of());
    ...
    @Test
    public void testGetParallelColumnMaskDoesNothingIfNotConfigured()
    {
        InstrumentedHttpClient httpClient = createMockHttpClient(
                OPA_SERVER_URI,
                request -> {
        ...
        Map<ColumnSchema, ViewExpression> result = authorizer.getTableColumnMasks(TEST_SECURITY_CONTEXT, TEST_COLUMN_MASKING_TABLE_NAME,
    ...
    @Test
    public void testGetParallelColumnMaskThrowsForIllegalResponse()
    {
        ...
        Consumer<OpaAccessControl> methodUnderTest = authorizer -> authorizer.getTableColumnMasks(TEST_SECURITY_CONTEXT, TEST_COLUMN_MASKING_TABLE_NAME, tableColumnSchemas);
    ...
    private void testGetParallelColumnMasks(Map<ColumnSchema, String> columnResponseContent, Map<ColumnSchema, OpaViewExpression> expectedResult)
    {
        InstrumentedHttpClient httpClient = createMockHttpClient(
                OPA_COLUMN_MASKING_URI,
        ...
        Map<ColumnSchema, ViewExpression> result = authorizer.getTableColumnMasks(TEST_SECURITY_CONTEXT, TEST_COLUMN_MASKING_TABLE_NAME, ImmutableList.copyOf(columnResponseContent.keySet()));
